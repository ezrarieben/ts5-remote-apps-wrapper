...
> **NOTE:** You can only use API keys generated by the API as described under [ Basic API connection](#basic-api-connection).<br/>
...
An easy way to figure out and track events that are being captured by the API Wrapper is to enable the `api.tsEventDebug` when instantiating the API wrapper:
```javascript
...
| Event                         | Description                                                                               |
| ----------------------------- | ----------------------------------------------------------------------------------------- |
| `tsOnAuth`                    | Called after the API wrapper sent an authentication request that was successfull          |
| `tsOnChannel`                 | Called after connecting to a server to give API info on channels available on new server. |
...
| `tsOnGroupInfo`               | Called after connecting to a server to give API info on groups available on new server.   |
